...
def EvaluationPlus3(platform):
    ...
    result_dir = os.path.join(
        autosparse_prefix, "python", "experiments", platform + "_evaluation_plus3"
    )
    ...
    for mtx in mtx_names:
        for method in search_methods:
            ...
            i_3 = Axis(int(num_row), ModeType.DENSE, "i")
            j_3 = Axis(int(num_col), ModeType.COMPRESSED, "j")
            """Tensor declaration"""
            A = Tensor((i, j), is_sparse=True)
            B = Tensor((i_, j_), is_sparse=True)
            C = Tensor((i_2, j_2), is_sparse=True)
            """Calculation declaration"""
            D = Compute(A + B + C, format = (i_3, j_3), is_sparse = True)
