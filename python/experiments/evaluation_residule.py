...
def EvaluationPlus3(platform):
    ...
    for mtx in mtx_names:
        for method in search_methods:
            ...
            j_2 = Axis(int(num_col), ModeType.DENSE, "j")
            """Tensor declaration"""
            A = Tensor((i, j), is_sparse=True)
            B = Tensor((i_,j_), is_sparse=False)
            C = Tensor((i_2,j_2), is_sparse=False)
            ...
            sch = AutoTune(D, method = method, population_size=100, trial = 100,
                           ...
                           save_dirpath = os.path.join(autosparse_prefix, "python", "experiments", platform + "_evaluation_plus3"))
            func = Build(sch)
