...
class LoadHybridSparseMatrixScheduleDataSet(torch.utils.data.Dataset):
    ...
    def data_process(self, file_path, dataset_dirname_prefixs_lst):
        ...
        data = []
        for mtx_name in tqdm(
            mtx_names, ncols=110, desc="Load sparse matrix with it's schedule: "
        ):
...
if __name__ == "__main__":
    ...
    data = dataset.load_test_data(
        os.path.join(root, "dataset", "validation.txt"), ["epyc_7543/spmm"]
    )
