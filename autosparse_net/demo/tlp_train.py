...
def train(train_loader, val_dataloader, device, is_recover = False):
    ...
    if is_recover:
        ...
        tlp_models = sorted(glob.glob(os.path.join(args.save_folder, "*")), 
                            key=lambda x: extract_numbers(x.split('/')[-1])[0])
        if len(tlp_models) > 0:
            with open(tlp_models[-1], 'rb') as f:
...
if __name__ == "__main__":
    print(sys.argv[0])
    intrin_name = sys.argv[1]
    del sys.argv[1]
    
    measured_path = f'/home/zhwang/TensorizeSet/{intrin_name}_dataset/measured_records'
    
    dataset_save_path = '/home/zhwang/TensorizeSet/tlp_records/tlp_dataset'
    # USE packed Dataloader
    train_save_dataloader_name = f'tlp_{intrin_name}_train_and_val.pkl.DataLoader'
    # USE pkl
    train_save_pkl_name = f'tlp_{intrin_name}_train_and_val.pkl'
    ...
    model_save_path = f'/home/zhwang/TensorizeSet/tlp_records/tlp_models/{intrin_name}'
    
    task_info_path = '/home/zhwang/TensorizeSet/task_info'
    
    embedding_utils = Embedding(table_save_path, table_save_name)
    # print(embedding_utils)
    parser = argparse.ArgumentParser()
    parser.add_argument("--save_folder", type=str, default=f'{model_save_path}')
    ...
    parser.add_argument("--step_size", type=int, default=embedding_utils.max_seq_len)
    parser.add_argument("--fea_size", type=int, default=embedding_utils.max_embedding_len)
