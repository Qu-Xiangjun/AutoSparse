...
////    cout<<" \nstandard daviation = "<< sd<<endl;
   ...
   QValue *Y = (QValue*)malloc(20*1000000*sizeof(QValue));
    ...
    // for(int i = 0 ; i< A.nnz ; i++) A.colInd[i]++;
    // for(int i = 0 ; i<= A.rows ; i++) A.rowPtr[i]++;
    // cout << A.colInd[0] << " " << A.colInd[1] << " " << A.colInd[2] << endl;
    // cout << A.rowPtr[0] << " " << A.rowPtr[1] << " " << A.rowPtr[2] << endl;
    ...
    for( n=8 ; n <= 128 ; n*=4) 
    {
        ...
        // TODO: mkl_scsrmm 是base-1的函数，A.colInd, A.rowPtr默认是base-0的内容
        /**
         * transa N indicate none change for A, T indicate transpose, C indicate A^H
         * m: indicate number of rows in matrix A
         * k: indicate number of cols in matirx A
         * n: indicate number of cols in matrix B and C
         * matdescra: describes the relevant characteristic of the matrix A,
         *            which is decribed in https://www.intel.com/content/www/us/en/docs/onemkl/developer-reference-fortran/2024-1/interface-consideration.html
         * 
         * 
         */
...
/////        cout<<"TTAAGG,"<<argv[1]<<","<<n<<","<<(double)2*(double)A.nnz*(double)n/elapsed/1000000000<<","<<endl;
	...
	cout<<(double)2*(double)A.nnz*(double)n/elapsed/1000000000<<", Time = "<< elapsed*1000 << " ms" << endl;
