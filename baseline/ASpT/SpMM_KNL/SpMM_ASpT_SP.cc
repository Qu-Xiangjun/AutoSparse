...
////sc = atoi(argv[2]);
sc=128;
        fp = fopen(argv[1], "r"); // 打开mtx文件
        fgets(buf, 300, fp); // 这一行从已打开的文件中读取一行内容，最多读取 300 个字符，并将结果存储到字符数组 buf 中
		// 这一行检查读取到的内容是否包含 "symmetric" 或 "Hermitian" 字符串，如果有，则将 sflag 设置为 1，表示对称性
        if(strstr(buf, "symmetric") != NULL || strstr(buf, "Hermitian") != NULL) sflag = 1; // symmetric
        else sflag = 0;
		//  这一行检查读取到的内容是否包含 "pattern" 字符串，如果有，则将 nflag 设置为 0，表示无值（non-value）
        if(strstr(buf, "pattern") != NULL) nflag = 0; // non-value
        else if(strstr(buf, "complex") != NULL) nflag = -1; // 这一行检查读取到的内容是否包含 "complex" 字符串，如果有，则将 nflag 设置为 -1
        else nflag = 1; // 正常情况为1
		...
		// 每次循环都会读取文件中的一行内容，直到遇到不包含百分号 "%" 的行为止。
		// 在每次循环中，变量 pre_count 会递增，用于记录已读取的行数。
        while(1) {
                pre_count++;
                fgets(buf, 300, fp);
        ...
        nr0 = nr;
        ne *= (sflag+1); // 正常情况sflag = 0
        nr = CEIL(nr,BH)*BH; // 行数对齐128
	npanel = CEIL(nr,BH);
		...
		temp_v[i].grp = INIT_GRP;
                temp_v[i].row--; temp_v[i].col--; // 注意mtx文件中坐标是1开始的
...
temp_v[i].val = 1.0f;
        ...
        qsort(temp_v, ne, sizeof(struct v_struct), compare0); // 对coo排序
...
////gettimeofday(&tt3, NULL);
	...
	fprintf(stdout, " gen: %f ms,%f ms,", elapsed0*1000, p_elapsed*1000);
...
#define VALIDATE
  ...
  sc = 128;
        int num_diff=0;
        for(int i=0;i<nr*sc;i++) {
                FTYPE p1 = vout_gold[i]*ITER; FTYPE p2 = vout[i];
...
//      fprintf(stdout, "ne : %d\n", gold_ne);
        ...
        fprintf(stdout, "GFlops: %f,", (double)ne*2*8*ITER/elapsed[0]/1000000000);
        fprintf(stdout, "%f,%f \n", (double)ne*2*32*ITER/elapsed[1]/1000000000, (double)ne*2*128*ITER/elapsed[2]/1000000000);
	// fprintf(stdout, "%f\n", p_elapsed/(elapsed[2]/ITER));
	fprintf(stdout, "Time = %f ms, %f ms, %f ms\n", elapsed[0]*1000, elapsed[1]*1000, elapsed[2]*1000);
