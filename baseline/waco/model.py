...
class ResNetBaseSpMM(nn.Module):
    BLOCK = None
    LAYERS = ()
    INIT_DIM = 32 
    PLANES = (16,32,64,64)
    ...
    def network_initialization(self, in_channels, out_channels, D):
        ...
        # Super Schedule
        self.isplit = nn.Embedding(17, 32) # 词汇表的大小， 嵌入向量的维度大小
    ...
    def forward(self, x: ME.SparseTensor, x2, y):
        # Concat - Final
        x = self.embed_sparse_matrix(x, x2)
        y = self.embed_super_schedule(y)
        xy = torch.cat((x,y), dim=1)
        xy = self.final(xy)
        return xy
...
class ResNet14SpMM(ResNetBaseSpMM):
    LAYERS = (1, 1, 1, 1)

class ResNetBaseSpMV(nn.Module):
    BLOCK = None
    LAYERS = ()
    INIT_DIM = 32 
    PLANES = (16,32,64,64)
    ...
    def network_initialization(self, in_channels, out_channels, D):
        ...
        self.formatk0 = nn.Embedding(2, 32)
        self.parchunk = nn.Embedding(9, 32) # For OpenTuner
        self.order = nn.Linear(16, 32) #6x6 Permutation
    ...
    def embed_super_schedule(self, y) :
        ...
        isplit = self.isplit(y[:, 0].long())
        ksplit = self.ksplit(y[:, 1].long())
        order = self.order(y[:, 2:18])
        i1f = self.formati1(y[:, 18].long())
        i0f = self.formati0(y[:, 19].long())
        k1f = self.formatk1(y[:, 20].long())
        k0f = self.formatk0(y[:, 21].long())
        #pidx = self.paridx(y[:, 43].long())
        #pnum = self.parnum(y[:, 44].long())
        pchk = self.parchunk(y[:, 24].long())
        y = torch.cat((isplit,ksplit,order,i1f,i0f,k1f,k0f,pchk), dim=1)
        y = self.schedule_embedding(y)
...
class ResNet14SpMV(ResNetBaseSpMV):
    LAYERS = (1, 1, 1, 1)


class ResNetBaseSDDMM(nn.Module):
    BLOCK = None
    LAYERS = ()
    INIT_DIM = 32 
    PLANES = (16,32,64,64)
    ...
    def network_initialization(self, in_channels, out_channels, D):
        ...
        # Super Schedule
        self.isplit = nn.Embedding(17, 32)
        self.jsplit = nn.Embedding(17, 32)
        self.ksplit = nn.Embedding(8, 32)
        self.formati1 = nn.Embedding(2, 32)
        self.formati0 = nn.Embedding(2, 32)
        self.formatj1 = nn.Embedding(2, 32)
        self.formatj0 = nn.Embedding(2, 32)
        self.paridx = nn.Embedding(6, 32) # For OpenTuner
        self.parchunk = nn.Embedding(6, 32) # For OpenTuner
        self.order = nn.Linear(36, 32) #6x6 Permutation
    ...
    def embed_super_schedule(self, y) :
        # Super Schedule
        isplit = self.isplit(y[:, 0].long())
        jsplit = self.jsplit(y[:, 1].long())
        ksplit = self.ksplit(y[:, 2].long())
        order = self.order(y[:, 3:39])
        i1f = self.formati1(y[:, 39].long())
        i0f = self.formati0(y[:, 40].long())
        j1f = self.formatj1(y[:, 41].long())
        j0f = self.formatj0(y[:, 42].long())
        pidx = self.paridx(y[:, 43].long())
        pchk = self.parchunk(y[:, 45].long())
        y = torch.cat((isplit,jsplit,ksplit,order,i1f,i0f,j1f,j0f,pidx,pchk), dim=1)
        y = self.schedule_embedding(y)
        return y
...
class ResNet14SDDMM(ResNetBaseSDDMM):
    LAYERS = (1, 1, 1, 1)
