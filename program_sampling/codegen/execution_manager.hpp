...
int compute_(taco_tensor_t *C, taco_tensor_t *A, taco_tensor_t *B, taco_tensor_t *Cd, taco_tensor_t *Ad, taco_tensor_t *Bd) {
  ...
  int* __restrict A4_crd = (int*)(A->indices[3][1]);
  ...
  int32_t k0A = A1_pos[0];
  int32_t pA1_end = A1_pos[1];

    int  cnt = 0;
    
  while (k0A < pA1_end) {
    int32_t k0 = A1_crd[k0A];
    int32_t A1_segend = k0A + 1;
    while (A1_segend < pA1_end && A1_crd[A1_segend] == k0) {
      A1_segend++;
    }
    for (int32_t i1A = A2_pos[k0A]; i1A < A2_pos[A1_segend]; i1A++) {
      int32_t i1 = A2_crd[i1A];
      for (int32_t k1 = 0; k1 < B1_dimension; k1++) {
        int32_t k1A = i1A * A3_dimension + k1;
        int32_t k0B = k1 * B2_dimension + k0;
        int32_t i0A = k1A;
        int32_t pA4_end = k1A + 1;

        while (i0A < pA4_end) {
          int32_t i0 = A4_crd[i0A];
          float A_val = A_vals[i0A];
          i0A++;
          while (i0A < pA4_end && A4_crd[i0A] == i0) {
            A_val += A_vals[i0A];
            i0A++;
          }
          int32_t i0C = i1 * C2_dimension + i0;
          for (int32_t j1 = 0; j1 < B3_dimension; j1++) {
            int32_t j1C = i0C * C3_dimension + j1;
            int32_t j1B = k0B * B3_dimension + j1;
            for (int32_t j0 = 0; j0 < B4_dimension; j0++) {
              int32_t j0C = j1C * C4_dimension + j0;
              int32_t j0B = j1B * B4_dimension + j0;
              C_vals[j0C] = C_vals[j0C] + A_val * B_vals[j0B];
              if (i0 == 0 and i1 == 0 and j1 == 0 and j0 == 0)
              {
                stringstream ss;
                ss << i0 << "," << k0 <<","<<k1<<","<<i1;
                fwrite2file(ss.str(), "compute_index.txt");
                cnt++;
              }
            }
          }
